    ...
    def callback_survivor1(self, data):
        if not self.flag1:
            try: 
                ...
                result = self.reset_model(state_msg)
                if result.success: 
                    self.flag1 = True
                    print("callback_survivor1 success")
    ...
    def callback_survivor2(self, data):
        if not self.flag2:
            try: 
                ...
                if result.success: 
                    self.flag2 = True
                    print("callback_survivor2 success")
            except(rospy.ServiceException) as e: print(e)
        else:
            return
    ...
    def callback_survivor3(self, data):
        if not self.flag3:
            try: 
                ...
                if result.success: 
                    self.flag3 = True
                    print("callback_survivor3 success")
            except(rospy.ServiceException) as e: print(e)
        else:
            return
    ...
    def callback_survivor4(self, data):
        if not self.flag4:    
            try: 
                ...
                result = self.reset_model(state_msg)
                if result.success: 
                    print("callback_survivor4 success")
                    self.flag4 = True
            except(rospy.ServiceException) as e: print(e)
        else:
            return
